...
namespace BasicEyetrackingSample
{
    public partial class MainForm : Form
    {
        ...
        private IGazeDataItem _dataItem;
        ...
        private void _connectedTracker_GazeDataReceived(object sender, GazeDataEventArgs e)
        {
            ...
            stopwatch.Start();
            Console.Write("T " + stopwatch.Elapsed.ToString());
            Console.Write('\n');*/
            // Send the gaze data to the track status control.
            var gd = e.GazeDataItem;
            //System.Threading.Thread.Sleep(1000);
            Console.WriteLine("LG {0:F20} {0:F20}", e.GazeDataItem.LeftGazePoint2D.X, e.GazeDataItem.LeftGazePoint2D.Y);
            Console.WriteLine("RG {0:F20} {0:F20}", e.GazeDataItem.RightGazePoint2D.X, e.GazeDataItem.RightGazePoint2D.Y);
            Console.WriteLine("D {0:F20} {0:F20}", e.GazeDataItem.LeftPupilDiameter, e.GazeDataItem.RightPupilDiameter);
            Console.WriteLine("V {0} {0}", e.GazeDataItem.RightValidity, e.GazeDataItem.LeftValidity);
            DateTime epoch = new DateTime(1970, 1, 1, 0, 0, 0, DateTimeKind.Utc);
            epoch = epoch.AddMilliseconds(e.GazeDataItem.Timestamp);// your case results to 4/5/2013 8:48:34 AM
            Console.WriteLine("T {0:F20}", epoch);
            /*Console.WriteLine("LGY,{0:F20}", e.GazeDataItem.LeftGazePoint2D.Y);
            Console.WriteLine("RGX,{0:F20}", e.GazeDataItem.RightGazePoint2D.X);
            Console.WriteLine("RGY,{0:F20}", e.GazeDataItem.RightGazePoint2D.Y);
            Console.WriteLine("LGX,{0:F20}", e.GazeDataItem.LeftGazePoint3D.X);
            Console.WriteLine("LGY,{0:F20}", e.GazeDataItem.LeftGazePoint3D.Y);
            Console.WriteLine("LGZ,{0:F20}", e.GazeDataItem.LeftGazePoint3D.Z);
            Console.WriteLine("RGX,{0:F20}", e.GazeDataItem.RightGazePoint3D.X);
            Console.WriteLine("RGY,{0:F20}", e.GazeDataItem.RightGazePoint3D.Y);
            Console.WriteLine("RGZ,{0:F20}", e.GazeDataItem.RightGazePoint3D.Z);
            Console.WriteLine("LPX,{0:F20}", e.GazeDataItem.LeftEyePosition3D.X);
            Console.WriteLine("LPY,{0:F20}", e.GazeDataItem.LeftEyePosition3D.Y);
            Console.WriteLine("LPZ,{0:F20}", e.GazeDataItem.LeftEyePosition3D.Z);
            Console.WriteLine("RPX,{0:F20}", e.GazeDataItem.RightEyePosition3D.X);
            Console.WriteLine("RPY,{0:F20}", e.GazeDataItem.RightEyePosition3D.Y);
            Console.WriteLine("RPZ,{0:F20}", e.GazeDataItem.RightEyePosition3D.Z);
            Console.WriteLine("D {0:F20} {0:F20}", e.GazeDataItem.LeftPupilDiameter, e.GazeDataItem.RightPupilDiameter);
            Console.WriteLine("RPD,{0:F20}", e.GazeDataItem.RightPupilDiameter);
            Console.WriteLine("LV,{0}", e.GazeDataItem.LeftValidity);
            Console.WriteLine("RV,{0}", e.GazeDataItem.RightValidity);
            Console.WriteLine("T,{0:F20}", e.GazeDataItem.Timestamp);
            */
